...
it('records the quantity used to decide the case rather than an unchanged baseline',()=>{
 const indices={3:0,4:4,5:3,7:2,10:3,12:5};for(const [n,index] of Object.entries(indices))for(const c of missions[n-1].investigation.cases)expect(c.evidenceIndex).toBe(index);
